...
<article class="page-container">
	...
	<form class="form form-horizontal" action="{{url('admin/picture/'.$picture->id.'')}}" method="post" enctype="multipart/form-data" id="form-article-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$picture->pic_name}}" placeholder="" id="articletitle" name="pic_name">
			</div>
		</div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">请选择图片：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input name="new_pic_src" type="file" multiple="true">

            </div>
            @if($picture->id)
                <img src="{{url('public/uploads')}}/{{$picture->pic_src}}" width="150" style="margin-left: 20px; margin-top: 5px;">
                <input type="hidden" name="pic_src" value="{{$picture->pic_src}}">
            @endif
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">图片链接</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$picture->pic_url}}" placeholder="例：www.baidu.com" id="author" name="pic_url">
            </div>
        </div>
		<div class="row cl">
